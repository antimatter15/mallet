...
function App() {
    ...
    React.useEffect(() => {
        let keyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowLeft') {
                let color = items[left].mu > items[right].mu ? 'gray' : '#007fff'
            ...
            >
                {Object.keys(items).length < 4 ? (
                    ...
                    >
                        <button
                            onClick={e => {
                                let [newLeft, newRight] = pickInterestingPair(left, right)
                                setLeft(newLeft)
                                setRight(newRight)
                            }}
                        >
                    ...
                    <>
                        <div id="left" style={{ width: '50%', padding: 100 }} key={left}>
                            {items[left].name}
                        </div>
                        <div id="right" style={{ width: '50%', padding: 100 }} key={right}>
                            {items[right].name}
                        </div>
                    </>
                )}
                            ...
                            newLines.map((k, i) => {
                                let match =
                                    oldLines.length === newLines.length
                                        ? Object.keys(items).find(j => items[j].index === i)
                                        : Object.keys(items).find(j => items[j].name === k)
                                if (match) {
                ...
                >
                    ...
                    >
                        {text
                            .split('\n')
                            .map((k, i) => {
                                let m = Object.values(items).find(j => j.name === k)
                                if (!m) return ''
